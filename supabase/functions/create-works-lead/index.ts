...
async function notifySlack(supabase: SupabaseClient, leadId: string) {
  ...
  try {
    ...
    console.error("Works Slack notification failed", error);
    await supabase
      .from("works_leads")
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const insertion = await supabase.from("works_leads").insert(lead).select("id").single();
    ...
    if (insertion.error?.code === "23505") {
      const existing = await supabase
        ...
        .single();
      if (existing.error || !existing.data) throw new Error("Could not resolve Works lead retry");
      leadId = existing.data.id;
      created = false;
    } else if (insertion.error || !leadId) {
      throw new Error("Could not create Works lead");
    }
    ...
    console.error("Works lead creation failed", error);
