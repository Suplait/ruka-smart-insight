      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="relative mt-12">
          ...
          <div className="grid gap-10 md:grid-cols-3 md:gap-8">
            {oneContent.method.steps.map((step, index) => (
        ...
        >
          <h3 className="max-w-xl text-balance text-[30px] font-semibold leading-[1.12] tracking-[-0.03em] text-[#202230] sm:text-[36px]">
            {oneContent.method.technology.title}
          </h3>
          <p className="max-w-2xl text-pretty text-base leading-7 text-[#646b7b] sm:text-lg sm:leading-8">
            {oneContent.method.technology.copy}
          </p>
        </motion.div>

        <div className="mt-8 overflow-hidden rounded-xl border border-[#dfe3ea] bg-white">
          <div className="grid sm:grid-cols-2 lg:grid-cols-5">
            {oneContent.method.rail.map((item, index) => (
              <div key={item} className={`flex items-center gap-3 px-4 py-4 text-sm font-semibold text-[#454a59] sm:px-5 ${index < oneContent.method.rail.length - 1 ? "border-b border-[#e1e4ea] lg:border-b-0 lg:border-r" : ""}`}>
