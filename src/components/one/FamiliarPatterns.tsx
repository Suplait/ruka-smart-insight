...
type PatternKind = (typeof oneContent.familiar.patterns)[number]["kind"];
    ...
    <section id="dolor" className="px-5 py-20 sm:px-8 sm:py-24">
      <div className="mx-auto max-w-7xl">
        <div className="max-w-4xl">
          <p className="text-[10px] font-semibold tracking-[0.16em] text-[#5369eb]">{oneContent.familiar.eyebrow}</p>
          <h2 className="mt-5 text-balance text-[38px] font-semibold leading-[1.08] tracking-[-0.035em] text-[#171827] sm:text-[48px] lg:text-[52px]">
            {oneContent.familiar.title}
          </h2>
        </div>

        <div className="mt-12 grid border-t border-[#dfe3ea] md:grid-cols-2">
          {oneContent.familiar.patterns.map((pattern, index) => (
            <article
              key={pattern.number}
        ...
        <div className="flex flex-col gap-5 border-t border-[#dfe3ea] pt-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-lg font-semibold text-[#272936]">{oneContent.familiar.transition.title}</p>
            <p className="mt-1 text-sm leading-6 text-[#666d7d]">{oneContent.familiar.transition.copy}</p>
          </div>
          <Link to={ONE_CONTACT_PATH} className="inline-flex items-center gap-2 whitespace-nowrap text-sm font-semibold text-[#5369eb] transition-colors duration-150 hover:text-[#4054c5] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5369eb] focus-visible:ring-offset-4">
            {oneContent.familiar.transition.action} <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
