...
import {
  ...
  type OneFlockFrame,
  type OneFlockScene,
  type OneFlockSceneFactory,
} from "@/components/one/visual/oneFlockRuntime";
import type { OneFlockSceneProps } from "@/components/one/visual/oneFlockTypes";
...
const createMathematicalFlockScene: OneFlockSceneFactory = (renderer): OneFlockScene => {
  ...
  function updateWake(frame: OneFlockFrame, delta: number, pointerX: number, pointerY: number) {
  ...
  function simulate(frame: OneFlockFrame) {
...
export function OneFlock({ state, paused }: OneFlockSceneProps) {
  const { containerRef, failed } = useOneFlockScene(createMathematicalFlockScene, state, paused);
