...
export type VisualLabScene = {
  resize: (width: number, height: number, pixelRatio: number, compact: boolean) => void;
  render: (frame: VisualLabFrame) => void;
  dispose: () => void;
};

export type VisualLabSceneFactory = (renderer: THREE.WebGLRenderer) => VisualLabScene;

const stateTargets: Record<WorksVisualLabState, number> = {
  idle: 0,
  focus: 0.56,
  valid: 1,
};

export function useVisualLabScene(
  factory: VisualLabSceneFactory,
  state: WorksVisualLabState,
  paused: boolean,
