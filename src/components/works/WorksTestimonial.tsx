...
export function WorksTestimonial() {
  return (
    <section id="testimonio" className="border-y border-[#e1e4ea] bg-white px-5 py-16 sm:px-8 sm:py-20">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[180px_1fr] lg:items-start lg:gap-14">
        ...
        <blockquote>
          <p className="max-w-5xl text-balance text-[30px] font-medium leading-[1.22] tracking-[-0.025em] text-[#222432] sm:text-[38px] lg:text-[42px]">“{worksContent.testimonial.quote}”</p>
          <footer className="mt-7 text-sm">
            <strong className="font-semibold text-[#292b39]">{worksContent.testimonial.author}</strong>
            <span className="ml-2 text-[#6f7686]">{worksContent.testimonial.role}</span>
          </footer>
        </blockquote>
      </div>
    </section>
